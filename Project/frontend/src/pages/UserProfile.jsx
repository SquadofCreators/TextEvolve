...
import { useAuth } from '../contexts/AuthContext'; // Provides { user, token, logout, updateUserContext }
...
function UserProfile() {
    ...
    const handleSave = async () => {
        ...
        // Construct payload for profile update
        let payload;
        try {
            payload = {
        ...
        try {
            ...
            // If password fields are provided then call the change password endpoint
            if (formData.password) {
                try {
                    await userService.changePassword({ newPassword: formData.password });
                } catch (pwError) {
                    console.error("Password change failed (Raw Error):", pwError);
                    // Optionally, you can decide to rollback changes or simply inform the user
                    setUpdateError('Profile updated, but password change failed. Please try again.');
                    // You might want to return here if you want to block success when password update fails
                }
            }
            setEditing(false);
    ...
    if (fetchError) {
        return (
            <div className="flex flex-col items-center justify-center h-screen text-center p-4">
                ...
                <button onClick={fetchUserData} className="mt-4 px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 transition text-sm">
                    Retry
                </button>
                {!authUser && (
                    <Link to="/login" className="mt-2 px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition text-sm">
                        Go to Login
                    </Link>
                )}
            </div>
            ...
            <div className="">
                    ...
                    <div className='flex flex-col items-center md:items-start justify-center'>
                        {editing ? (
                            <input
                                ...
                                className="mt-1 text-center block w-full px-4 py-2 border rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-orange-500 text-3xl font-bold"
                ...
                <div className="mb-6">
                    ...
                    {editing ? (
                        <textarea
                            ...
                            className="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-orange-500"
                        ></textarea>
                    ) : (
                        <p className={`text-gray-600 dark:text-gray-300 whitespace-pre-wrap ${!userDisplayData?.bio ? 'italic text-gray-400 dark:text-gray-500' : ''}`}>
                ...
                {editing && (
                    <div className="mb-6 space-y-4 border-b border-gray-300 dark:border-gray-700 pb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={handleChange}
                                    className="mt-1 block w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-1 focus:ring-orange-500"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                        ...
                        <div>
                            <h3 className="text-lg font-medium text-gray-800 dark:text-gray-200 mb-2 border-t border-gray-300 dark:border-gray-600 pt-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="mt-1 block w-full px-3 py-2 pr-10 border rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-1 focus:ring-orange-500"
                                    />
                                    <button
                                        type="button"
                                        title={showPassword ? "Hide password" : "Show password"}
                                        className="absolute inset-y-0 right-0 top-6 pr-3 flex items-center text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                Note: Your password will be updated immediately after you save changes.
                            </p>
                        </div>
                        {updateError && (
                        ...
                        <div className="flex justify-end items-center space-x-3 pt-4">
                            <button
                                ...
                                disabled={isUpdating}
                                className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500 transition duration-200 text-sm font-medium disabled:opacity-50"
                            >
                                <FiX className="inline mr-1 mb-0.5"/> Cancel
                            </button>
                            <button
                                onClick={handleSave}
                                disabled={isUpdating}
                                className="w-max px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 disabled:opacity-50 transition duration-200 text-sm font-medium flex items-center justify-center gap-1"
                            >
                                {isUpdating ? (
                                    <>
                                    ...
                                    <>
                                        <FiSave className="inline mr-1 mb-0.5"/> Save Changes
                                    </>
                                )}
                            </button>
                        </div>
                ...
                {!editing && (
                    <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
                        <div>
                            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Position</h3>
                            <p className="mt-1 text-gray-700 dark:text-gray-300">
                                {userDisplayData?.position || <span className="italic text-gray-400 dark:text-gray-500">Not Provided</span>}
                            </p>
                        </div>
                        <div>
                            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Company</h3>
                            <p className="mt-1 text-gray-700 dark:text-gray-300">
                                {userDisplayData?.company || <span className="italic text-gray-400 dark:text-gray-500">Not Provided</span>}
                            </p>
                        </div>
                        <div>
                            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Location</h3>
                            <p className="mt-1 text-gray-700 dark:text-gray-300">
                                {userDisplayData?.location || <span className="italic text-gray-400 dark:text-gray-500">Not Provided</span>}
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="mb-6">
                    <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-2">Security</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div>
                            <h4 className="font-medium text-gray-600 dark:text-gray-400">Last Login Time</h4>
                            <p className="mt-1 text-gray-700 dark:text-gray-300">{formatDate(userDisplayData?.lastLoginAt)}</p>
                        </div>
                        <div>
                            <h4 className="font-medium text-gray-600 dark:text-gray-400">Last Login IP Address</h4>
                            <p className="mt-1 text-gray-700 dark:text-gray-300">{userDisplayData?.lastLoginIp || "N/A"}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-6 flex flex-col sm:flex-row justify-end items-center gap-3">
                    {!editing && (
                        <button
                            onClick={() => { setEditing(true); setUpdateError(''); setPicUploadStatus({ message: '', type: 'idle'}); }}
                            className="w-full sm:w-auto order-last sm:order-first px-5 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 transition duration-200 text-sm font-medium flex items-center justify-center gap-2"
                        >
                    ...
                    <button
                        onClick={logout}
                        className="w-full sm:w-auto px-5 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition duration-200 text-sm font-medium flex items-center justify-center gap-2"
                    >
                        <FiLogOut/> Logout
                    </button>
                </div>
